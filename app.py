...
# 1. Configuration (የአንተ መረጃዎች በትክክል ተቀምጠዋል)
...
# 4. Data Fetching (በየ 60 ሰከንዱ ብቻ ዳታ እንዲጠይቅ ተደርጓል)
...
# 5. ICT Strategy (FVG Detection)
def check_signals(df, symbol):
    if df is None or len(df) < 5: return
    last = df.iloc[-1]
    third = df.iloc[-3]
    price = last['close']
    if last['low'] > third['high']: # Bullish FVG
...
if df is not None:
    ...
    # Candlestick Chart (በምስል 4 ላይ የነበረው ውበት)
    ...
    st.warning("ዳታው ለጥቂት ሰከንዶች ቆይቷል። እባክህ ለ 60 ሰከንድ ገጹን አትንካው (API Limit)...")

# 7. Auto-Refresh (በየ 1 ደቂቃው)
time.sleep(60)
st.rerun()
